...
namespace ExcelShSy.Features.Services
{
    ...
    public class SyncPrice : IExecuteOperation
    {
        ...
        void ProcessPage(IExcelPage page)
        {
            ...
            {
                var article = worksheet.GetValue(row, headers.articleColumn)?.ToString();

                if (article == null) continue;
                if(!_dataProduct.Price.ContainsKey(article))
                {
                    product.Add(article);
                    continue;
                }

                worksheet.WriteCell(row, headers.neededColumn, _dataProduct.Price[article]);
            }
